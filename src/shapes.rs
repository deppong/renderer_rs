...
impl RLine {
    ...
    pub fn draw(&self, framedata: &mut Vec<u8>, width: u32, height: u32) {
        if (self.y1 - self.y0).abs() < (self.x1 - self.x0).abs() {
            if self.x0 > self.x1 {
                Self::draw_low(self, framedata, width, height, self.x1, self.y1, self.x0, self.y0);
            } else {
                Self::draw_low(self, framedata, width, height, self.x0, self.y0, self.x1, self.y1);
            }
        } else {
            if self.y0 > self.y1 {
                Self::draw_high(self, framedata, width, height, self.x1, self.y1, self.x0, self.y0);
            } else {
                Self::draw_high(self, framedata, width, height, self.x0, self.y0, self.x1, self.y1);
            }
        }
    }

    fn draw_low(&self, framedata: &mut Vec<u8>, width: u32, height: u32, x0: f32, y0: f32, x1: f32, y1: f32) {
        let dx = x1 - x0;
        let mut dy = y1 - y0;
    ...
    fn draw_high(&self, framedata: &mut Vec<u8>, width: u32, height: u32, x0: f32, y0: f32, x1: f32, y1: f32) {
        let mut dx = x1 - x0;
        let dy = y1 - y0;
